...
contract RealboxTokenTimelock is Ownable {
    ...
    // 8%: 5% at TGE, remaining lock 4 months, then vesting linear for 12 months
    function unlockedPrivateSales1(uint256 _timestamp) public view returns (uint256 _unlockedAmount) {
        return _monthlyUnlockedAt(80, 5, 4 * 30 days, 12, _timestamp);
    }

    // 6%: 5% at TGE, remaining lock 3 months, then vesting linear for 11 months
    function unlockedPrivateSales2(uint256 _timestamp) public view returns (uint256 _unlockedAmount) {
        return _monthlyUnlockedAt(60, 5, 3 * 30 days, 11, _timestamp);
    }
    ...
    // 6%: lock 2 years, then vesting linear for 12 months
    function unlockedMarketing(uint256 _timestamp) public view returns (uint256 _unlockedAmount) {
        return _monthlyUnlockedAt(60, 0, 24 * 30 days, 12, _timestamp);
    }
